...
function toUpper(word) {
  // check if the string has a charAt method (i.e., it is a string and not an integer)
  if (typeof word.charAt === 'function') {
    // convert the first character to uppercase and concatenate it with the rest of the string (slice(1))
    return word.charAt(0).toUpperCase() + word.slice(1);
  }
  // if the argument is not a string, return the original value
  return word;
}
...
const getLongDate = (date, timezone) => {
  ...
  // return a formatted date string using the Intl.DateTimeFormat object
  // with the following options: weekday (long), day (numeric), month (short), year (numeric), hour (numeric), minute (numeric), timeZone (timezone)
