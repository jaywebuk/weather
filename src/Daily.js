...
const Daily = ({ data }) => {
    ...
    const getDay = (thisDay) => {
        for (const key of Object.keys(thisDay)) {
            ...
            days.push(
                <div style={{ display: "flex", minWidth: "fit-content" }} key={`daily-${key}`}>
                    <section className={styles.day} onClick={(e) => handleClick(e, key)} data-hidden={key} role="button" tabIndex={key}>
                        <p>{currentDate}</p>
                        <img src={weatherIcon} alt="Weather Icon" title={toUpper(weatherDescription)} />
                        <div className={styles.wind}>
                            <img src={wind} style={{ rotate: thisDay[key]["wind_deg"] + "deg" }} alt="" title={"From the " + getCardinals(thisDay[key]["wind_deg"])} />
                            <p title={toUpper(getWind(Math.round(thisDay[key]["wind_speed"])))}>{Math.round(thisDay[key]["wind_speed"])} mph</p>
                        </div>
                        <div className={styles.dailyTemp}>
                            <img src={sun} alt="" />
                            <p>
                                {convertTemp(thisDay[key]["temp"]["day"])}&deg;C / {Math.round(thisDay[key]["temp"]["day"])}&deg;F
                            </p>
                        ...
                        <div className={styles.dailyTemp}>
                            <img src={moon} alt="" />
                            <p>
                                {convertTemp(thisDay[key]["temp"]["night"])}&deg;C / {Math.round(thisDay[key]["temp"]["night"])}&deg;F
                            </p>
                        </div>
                    </section>
                    <section className={styles.hidden} ref={(e) => (hiddenDaySections.current[key] = e)}>
                        <p>{toUpper(thisDay[key]["weather"][0]["description"])}</p>
                        <p className={styles.wind}>
                            {toUpper(getWind(thisDay[key]["wind_speed"]))} from the {getCardinals(thisDay[key]["wind_deg"])}
                        </p>
                        <p>Pressure: {thisDay[key]["pressure"]} mb</p>
                        <p>Chance of Precipitation: {Math.round(thisDay[key]["pop"] * 100)}%</p>
                        <p>
                            Sunrise: {getShortTime(thisDay[key]["sunrise"])} Sunset: {getShortTime(thisDay[key]["sunset"])}
                        </p>
                    </section>
                </div>
            );
