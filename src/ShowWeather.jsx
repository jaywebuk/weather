...
function ShowWeather({ data, setLoading }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) => {
        console.error(error);
        setErrorMessage('Shucks, an error occurred. Please try again.'); // Set the error message
        setLongLoading(false); // Set the longLoading state variable to false
        clearTimeout(timeoutId); // Clear the timeout
        setAbortFetch(true); // Set the abortFetch state variable to true
        setLoading('hidden'); // Set the loading state variable to 'hidden'
        setRequestError(error.response.data); // Set the request error
      });
